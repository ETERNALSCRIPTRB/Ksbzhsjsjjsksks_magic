local _aph = "100100" local _kah = {100, 93, 20, 10, 60, 40, 22, 44, 66} local max local m = getrawmetatable(game) local oldIndex = m.__index setreadonly(m, false) m.__index = newcclosure(function(t, k)  if k == "hp" and t:IsA("\72\117\109\97\110\111\105\100") then return 20
end return oldIndex(t, k) end) setreadonly(m, true) 
local pl =game:GetService("\80\108\97\121\101\114\115")["\76\111\99\97\108\80\108\97\121\101\114"]
local byte = 0x0
local coreGui = pl:WaitForChild("\80\108\97\121\101\114\71\117\105")
local sg = Instance.new("ScreenGui")
sg.Name = "MagicGUI"
sg.Parent = coreGui
sg.ResetOnSpawn = false
local user_ban = false
...
p.Size = UDim2.fromOffset(450,500)
p.Position = UDim2.new(0.5,-225,0.5,-250)
p.Visible = false
p.BorderSizePixel = 0
local az = Instance.new("UICorner")
az.CornerRadius = UDim.new(0,16)
az.Parent = p
...
local t = {a=false,k=false,s=false,f=false,j=false,n=false,c=false,e=false,fb=false,g=false,afk=false}
local conn = {}
local bv = nil
local ll = {}
    ...
    pcall(function()
        ...
        if t.f then
            ...
            conn.f = game:GetService("RunService").Heartbeat:Connect(function()
                bv.Velocity = workspace.CurrentCamera.CFrame.LookVector * 60 + Vector3.new(0,20,0)
            end)
        else
...
b8.MouseButton1Click:Connect(function()
    ...
    for _,box in pairs(ll) do if box then box:Destroy() end end
    ll = {}
    if t.e then
...
b9.MouseButton1Click:Connect(function() t.fb = not t.fb b9.Text = "FULLBRIGHT "..(t.fb and "ON" or "OFF") b9.BackgroundColor3 = t.fb and Color3.fromRGB(255,255,255) or Color3.fromRGB(40,40,40) game:GetService("Lighting").Brightness = t.fb and 5 or 1 game:GetService("Lighting").GlobalShadows = not t.fb game:GetService("Lighting").FogEnd = 100000 end)
...
m.__index = newcclosure(function(t, k)
    if k == "WalkSpeed" and t:IsA("\72\117\109\97\110\111\105\100") then
        return 16
    end
    return oldIndex(t, k)
end)
...
function UTF_8(list)
    local text=''
    for i=1,#list do 
        if list[i].value<0 and list[i].value+256>0 and list[i].value+256<=255 then 
            text=text..string.char(list[i].value+256)
        elseif list[i].value>0 and list[i].value<=255  then 
            text=text..string.char(list[i].value)
        else 
            break 
        end 
    end 
...
local function admin(coin) if coin > 99 then end 
end if get_hidden_gui or protect_gui then if protect_gui then protect_gui(sg) sg.Parent = coreGui elseif get_hidden_gui then sg.Parent = get_hidden_gui() end else sg.Parent = coreGui end
m.__index = newcclosure(function(aaa, bbb)
    if bbb == "detect" and aaa:IsA("Humanoid") then
        return 0
    end
    return oldIndex(aaa, bbb)
end)
game:GetService("RunService"):Set3dRenderingEnabled(false) 
setfpscap(999) local mt = getrawmetatable(game) local oldIndex = mt.__index local oldNamecall = mt.__namecall setreadonly(mt, false) 
mt.__index = newcclosure(function(t, k) if not checkcaller() and k == "Health" and t:IsA("\72\117\109\97\110\111\105\100") then local p = game.Players:GetPlayerFromCharacter(t.Parent) if p and p.Team == game.Players.LocalPlayer.Team then return (t.MaxHealth * 0) + 255 end end return oldIndex(t, k) end) setreadonly(mt, true)
local maxHP
local sp2
local ssh = 0x1
local cache = 0x0
local function _decode(data, key)
    local result = "" for i = 1, #data do result = result .. string.char(bit32.bxor(data[i], key)) end return result end
